...
if [ $CHECKCOUNT -gt 10 ]
then
   echo "$CHECKCOUNT occurrences of the \"Full GC\" has been found in the last 5 minutes"| mailx -s "Alert: High GC on $(hostname)" -S "from=ReactorCX@jenkins.devops.lmvi.net"  "${RECIPIENTS}"
fi
}
...
GC_COLLECTION
MEMORY
FILESYSTEM && [ -s "${LOG_FILE}" ] && mailx -s "Alert: Almost Out of Disk Space on $(hostname)" -S "from=subham.devops@gmail.com"  "${RECIPIENTS}" < ${LOG_FILE} && rm -rf ${LOG_FILE}
